...
/*GPS_OnPlayerStateChange(playerid, oldstate, newstate)
{
	// GPS Navigator
	if(pNavState[playerid])
	{
...
// GPS Navigator
CMD:navlocate(playerid, params[])
{  
	/*if(!pData[playerid][pVerified])
	{
		return SendClientMessage(playerid, COLOR_SYNTAX, "SERVER: You can't use this command as you are not verified yet on the server.");
	}*/
	if(!pData[playerid][pGPS])
	{
	    return SendClientMessage(playerid, COLOR_SYNTAX, "You don't have a GPS. You can buy one at 24/7.");
	}
	if(isnull(params)) ShowDialogToPlayer(playerid, DIALOG_NAVLOCATE);
	else LocateMethod(playerid, params);
	return 1;
}

CMD:gnavclose(playerid, params[])
{  
	if(!pData[playerid][pVerified])
	{
		return SendClientMessage(playerid, COLOR_SYNTAX, "SERVER: You can't use this command as you are not verified yet on the server.");
...
public Refresh(playerid)
{
	...
	switch(GetPVarInt(playerid, "NavData"))
	{
		case 1:
		{
			pPos[0] = NavigatorData1[pNavID[playerid]][NavigatorX];
			pPos[1] = NavigatorData1[pNavID[playerid]][NavigatorY];
			pPos[2] = NavigatorData1[pNavID[playerid]][NavigatorZ];
